                        ...
                        <v-row align="center">
                            ...
                            <v-col cols="12" md="7">
                                <v-btn
                                    ...
                                    @click="addEntryDialog.enabled = true"
                                >
        ...
        <v-container>
            ...
            <template v-else>
                <template v-if="Object.keys(transactions).length > 0">
                    <div
                        class="my-6"
                        v-for="(day, i) in Object.keys(transactions)"
                        :key="`day-${i}`"
                    >
                        <v-row align="center">
                            <v-col
                                cols="6"
                                class="pl-6 font-weight-bold grey--text text-uppercase"
                            >
                                {{ day }}
                            </v-col>
                            <v-col
                                cols="6"
                                class="pr-6 text-right font-weight-bold grey--text text-uppercase text-h6"
                            >
                                <span
                                    :class="[
                                        {
                                            'green--text':
                                                transactions[day].amount > 0,
                                            'red--text':
                                                transactions[day].amount < 0
                                        }
                                    ]"
                        ...
                        <v-hover
                            ...
                            v-for="(transaction, x) in transactions[day].data"
                            :key="`transaction-${x}`"
                        >
                            <v-card class="rounded-lg mb-6 custom-shadow">
                                <v-row no-gutters align="center" class="pa-4">
                                    ...
                                    <v-col
                                        ...
                                        cols="12"
                                        md="2"
                                        class="text-center text-md-right text-h6"
                                    >
                                        <span
                ...
                <template v-else>
                    <v-sheet
                        ...
                        style="height: 500px"
                        class="rounded-xl blue-grey--text text--darken-1 flex-column d-flex align-center text-center justify-center font-weight-medium text-h6"
                    >
            ...
            <v-card>
                <v-form ref="addEntryForm" @submit.prevent="addEntry()">
                        ...
                        <v-row>
                            <v-col cols="6">
                                <v-menu
                                    ...
                                    v-model="addEntryDialog.dateMenu"
                                    offset-y
                                    min-width="290px"
                                >
                                    ...
                                    <v-date-picker
                                        v-model="fields.date.value"
                                        scrollable
                                        @input="addEntryDialog.dateMenu = false"
                                    >
                                    </v-date-picker>
                                </v-menu>
                            </v-col>
                            <v-col cols="6">
                                <v-menu
                                    :close-on-content-click="false"
                                    ref="timeMenu"
                                    v-model="addEntryDialog.timeMenu"
                                    offset-y
                                    min-width="290px"
                                >
                                    <template v-slot:activator="{ on, attrs }">
                    ...
                    <v-card-actions class="pa-6">
                        <v-spacer></v-spacer>
                        <v-btn
                            :disabled="addEntryDialog.loading"
                            class="font-weight-bold mr-2"
                            large
                            depressed
                            dark
                            @click="addEntryDialog.enabled = false"
                            color="blue lighten-3"
                            >Cancel</v-btn
                        >
                        <v-btn
                            :loading="addEntryDialog.loading"
                            class="font-weight-bold"
                            large
                            depressed
                            dark
        ...
        <v-dialog v-model="deleteTransactionDialog.enabled" width="500">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <strong>{{
                        deleteTransactionDialog.transaction.label
                    }}</strong
                    >?
                </v-card-text>
                <v-card-actions class="pa-6">
                    <v-spacer></v-spacer>
                    <v-btn
                        :disabled="deleteTransactionDialog.loading"
                        class="font-weight-bold mr-2"
                        large
                        depressed
                        dark
                        @click="deleteTransactionDialog.enabled = false"
                        color="blue lighten-3"
                        >Cancel</v-btn
                    >
                    <v-btn
                        @click="confirmDeleteTransaction()"
                        :loading="deleteTransactionDialog.loading"
                        class="font-weight-bold"
                        large
                        depressed
                        dark
...
export default class App extends BaseClass {
    ...
    fields = {
        label: {
            value: '',
            rule: [(v: string) => (v && v.length > 0) || 'Invalid label.']
        },
        date: {
            value: null,
            rule: [
                (v: string) =>
                    (v && /([0-9]+){4}-([0-9]+){2}-([0-9]+){2}/.test(v)) ||
                    'Invalid date format.'
            ]
        },
        time: {
    ...
    private async loadTransactions() {
        ...
        try {
            ...
            this.transactions = {};
            this.totalBalance = 0;
            transactions.data.data.forEach(
                (item: DatabaseTransactionRecord) => {
